...
public sealed class SettingsService : ISettingsService
{
    ...
    public SettingsService(ApplicationDbContext db, IFileStorageService files, IOptions<FileStorageSettings> fileSettings, UserManager<ApplicationUser> users, IOtpService otp)
        => (_db, _files, _fileSettings, _users, _otp) = (db, files, fileSettings.Value, users, otp);
    ...
    public async Task<IdentitySettingsDto> UpdateIdentityAsync(string tenantId, UpdateIdentitySettingsRequest request, CancellationToken ct = default)
    {
        ...
        foreach (var upload in request.NewPhotos)
        {
            ...
            var url = await _files.SaveAsync(upload.Content, upload.FileName, upload.ContentType, business.TenantId, _fileSettings.BusinessGalleriesFolder, ct);
